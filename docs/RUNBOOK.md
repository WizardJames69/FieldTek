...
> ⚠️ **The eight deferred workflow-template migrations live in `supabase/migrations-deferred/`, NOT `supabase/migrations/`** (relocated in PR-DB-4). They are a separate, dormant work stream and must never participate in normal production replay:
