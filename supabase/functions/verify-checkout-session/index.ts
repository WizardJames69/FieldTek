...
serve(async (req) => {
  ...
  try {
    ...
    if (!subscriptionId) {
      logStep("No subscription on checkout session yet", { status: session.status, payment_status: session.payment_status });
      return new Response(
        JSON.stringify({
          subscribed: false,
          tier: (session.metadata?.tier as string | undefined) ?? "trial",
    ...
    logStep("Retrieving subscription", { subscriptionId });
    ...
    const tier =
      ...
      ((session.metadata?.tier as string | undefined) ?? "starter");

    const subscriptionEnd = subscription.current_period_end
      ? new Date(subscription.current_period_end * 1000).toISOString()
      : null;

    const trialEnd = isTrialing && subscription.trial_end
