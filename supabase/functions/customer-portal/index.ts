...
serve(async (req) => {
  ...
  try {
    ...
    // H5: Only owners and admins can access the billing portal
    ...
    if (!["owner", "admin"].includes(tenantUser.role)) {
      logStep("Access denied - insufficient role", { role: tenantUser.role });
      return new Response(
        JSON.stringify({ error: "Only account owners and admins can manage billing" }),
