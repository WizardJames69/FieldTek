...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Successfully retrieved conversation token");
    ...
    if (userId) {
      supabase
        ...
        .insert({
          ...
          metadata: { action: "token_issued", agent_id: ELEVENLABS_AGENT_ID },
        })
        .then(({ error }) => {
          if (error) console.error("Failed to log conversation usage:", error.message);
        });
    }
