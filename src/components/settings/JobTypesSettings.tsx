...
export function JobTypesSettings() {
  ...
  const jobTypes = (settings?.job_types as string[]) || [];
  ...
  const updateMutation = useMutation({
    mutationFn: async (types: string[]) => {
      ...
      const { error } = await supabase
        ...
        .update({ job_types: types })
        .eq('tenant_id', tenant.id);

      if (error) throw error;
    },
    onSuccess: () => {
      toast({ title: 'Job types updated' });
  ...
  const addType = () => {
    ...
    if (jobTypes.includes(newType.trim())) {
      toast({
    ...
    updateMutation.mutate([...jobTypes, newType.trim()]);
    setNewType('');
  };

  const removeType = (type: string) => {
    updateMutation.mutate(jobTypes.filter((t) => t !== type));
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Define the types of jobs your team handles
        </CardDescription>
      </CardHeader>
